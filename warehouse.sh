...
source ~/warehouse-service-env.sh

#shellcheck source=/dev/null
source ~/service-env-devnet-warehouse-*.sh
...
ledger_snapshots_dir=$HOME/ledger-snapshots
...
identity_keypair=$HOME/identity/devnet-validator-identity-$ZONE.json
...
args=(
  ...
  --ledger "$ledger_dir"
  --log "$HOME/logs/solana-warehouse.log"
  --no-voting
  --skip-poh-verify
  ...
  --snapshots "$HOME/ledger-snapshots"
)
...
while true; do
  ...
  while true; do
    ...
    last_archive_epoch=$(cat $HOME/ledger-archive/epoch || true)
    if [[ -z "$last_archive_epoch" ]]; then
      if ! solana --url "$RPC_URL" epoch > $HOME/ledger-archive/epoch; then
        ...
        if ((minutes_since_last_ledger_archive % 60 == 0)); then
          ...
          if [[ -n $latest_snapshot ]]; then
            ...
            if [[ $latest_snapshot_slot = "$last_known_good_snapshot_slot" ]]; then
              ...
              ln "$latest_snapshot" $HOME/snapshot-check/
              if solana-ledger-tool --ledger $HOME/snapshot-check verify; then
    ...
    ledger_bounds="$(cat $HOME/ledger-archive/bounds.txt)"
