...
#shellcheck source=/dev/null
source ~/service-env-"$NETWORK"-validator-*.sh

identity_keypair=~/identity/"$NETWORK"-validator-identity-"$ZONE".json
identity_pubkey=$(solana-keygen pubkey "$identity_keypair")
vote_acct_keypair=~/identity/"$NETWORK"-vote-account-"$ZONE".json
echo vote acct = $vote_acct_keypair
ledger_dir=~/ledger
log_dir=~/logs
ledger_snapshot_dir=~/ledger-snapshots
...
if [[ -f $vote_acct_keypair ]]; then
  args+=(--vote-account $vote_acct_keypair)
else
  args+=(--no-voting)
fi

for av in ~/identity/validator-authorized-voter*.json; do
